...
public class Menu {
	...
	/**
	 * Adding launcher destructor 
	 * by using id input from user
	 */
 ...
 */
	public void missileLunch(){
		...
		int lId= s.nextInt();
		Launcher l = war.getMissileLaunchers().get(war.getMissileLaunchers().indexOf(new Launcher("L" + lId)));
	...
	public void launcherIntercept(){
		...
		for (Launcher launcher : launchers) {
			...
			try{
				...
				if(launchers.get(launchers.indexOf(new Launcher(lId))).destroyLauncher()){
	...
	public void missileIntercept(){
		...
		if (war.getInAirMissiles().size() > 0){
			...
			if (selectedDestractor.intercept(selectedMissile))
				System.out.println("MISSILE " + selectedMissile.getMissileId() + " WAS INTERCEPTED!");
			else
				System.out.println("MISSILE " + selectedMissile.getMissileId() + " WAS MISSED!");
		} else
		{
